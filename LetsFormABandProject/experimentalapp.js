$(() => {
  ...
  let columnArray = [column1, column2, column3, column4, column5, column6, column7, column8];
    ...
    let newNextImageSrc;
    let previousImageClass;
    let previousImageSrc;
    let newPreviousImageClass;
    let newPreviousImageSrc;
    let nextColumnImageClass;
    let nextColumnImageSrc;
    let newNextColumnImageClass;
    let newNextColumnImageSrc;
    let $allImages = $('.square').children();
    console.log($allImages);
    // call out individual columns:

    $($allImages).on('click', (event) => {
      for (event.currentTarget of columnArray) {
        console.log('this works');
        // find which column it is, index 0-7
        // console.log(columnarray);
        // console.log(column);
        imgParent = $(event.currentTarget).parent()
        columnIndex = columnarray.indexOf($(event.currentTarget.);
        // find column right before it in columnarray
        columnBefore = columnarray[columnIndex - 1];
        // console.log(columnBefore);
        columnAfter = columnarray[columnIndex + 1];

        for (image of column) {

          // $(image).on('click', (event) => {
          console.log(columnIndex);
          imageIndex = column.indexOf(image);
          // find previous image in column
          previousImage = column[imageIndex - 1];
          // find next image in column
          nextImage = column[imageIndex + 1];
          // Find image with same index but in previous column:
          if (columnBefore) {
            // console.log(columnBefore);
            previousColumnImage = columnBefore[imageIndex];
          }
          if (columnAfter) {
            nextColumnImage = columnAfter[imageIndex];
          ...
          // debugger;
          if ($(nextImage).attr('class') === 'boss' || $(nextImage).attr('class') === 'bandmate' || $current.attr('class') === 'boss' || $current.attr('class') === 'bandmate') {
            console.log('this is a boss or bandmate');
          } else {
            $(nextImage).off('click');
            $(nextImage).on('click', () => {
              // console.log(nextImage);
              nextImageClass = $(nextImage).attr('class');
              ...
              // debugger;
              newnextImageClass = $(nextImage).attr('class');
              newnextImageSrc = $(nextImage).attr('src');
          ...
          if ($(previousImage).attr('class') === 'boss' || $(previousImage).attr('class') === 'bandmate' || $current.attr('class') === 'boss' || $current.attr('class') === 'bandmate') {
            ...
            $(previousImage).on('click', () => {
              ...
              previousImageClass = $(previousImage).attr('class');
              previousImageSrc = $(previousImage).attr('src');
              ...
              // debugger;
              newPreviousImageClass = $(previousImage).attr('class');
              newPreviousImageSrc = $(previousImage).attr('src');
          ...
          if ($(nextColumnImage).attr('class') === 'boss' || $(nextColumnImage).attr('class') === 'bandmate' || $current.attr('class') === 'boss' || $current.attr('class') === 'bandmate') {
            ...
            $(nextColumnImage).on('click', () => {
              // console.log(previousImage);
              nextColumnImageClass = $(nextColumnImage).attr('class');
              nextColumnImageSrc = $(nextColumnImage).attr('src');
              // console.log(previousColumnClass);
              // console.log(previousColumnSrc);
              // debugger;
              ...
              // debugger;
              newNextColumnImageClass = $(nextColumnImage).attr('class');
              newNextColumnImageSrc = $(nextColumnImage).attr('src');
              // console.log(newpreviousImageClass);
              // console.log(newpreviousImageSrc);
              // debugger;
              // remove current class and image
